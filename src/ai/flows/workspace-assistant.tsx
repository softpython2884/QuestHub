...
export async function workspaceAssistant(
  ...
  const flow = ai.defineFlow(
    ...
    async (flowInput) => {
      ...
      const latestUserMessage = flowInput.history[flowInput.history.length - 1];
      ...
      if (user?.user?.uuid) {
        ...
        const mentionedProject = userProjects.find(p => latestUserMessage.content.toLowerCase().includes(p.name.toLowerCase()));
...
- **Context Awareness**: Use the provided context to inform your actions. If the user says "create a task for this project" and the context includes a project UUID, you should use it. The full conversation history is provided for context.
...
`;
      ...
      try {
        const { text, toolCalls } = await ai.generate({
          ...
          system: systemPrompt,
          history: flowInput.history as ChatMessage[],
          tools: toolsToUse,
        });
