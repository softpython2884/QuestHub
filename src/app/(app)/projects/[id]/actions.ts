...
import { generateTasks, type GenerateTasksInput, type GenerateTasksOutput } from '@/ai/flows/generate-tasks';
...
export async function toggleProjectUrgencyAction(prevState: ToggleProjectUrgencyFormState, formData: FormData): Promise<ToggleProjectUrgencyFormState> {
  ...
  try {
    ...
    const updatedProject = await dbUpdateProjectVisibility(projectUuid, isUrgent);
    if (!updatedProject) {
...
export interface GenerateTasksAIFormState {
  message?: string;
  error?: string;
  fieldErrors?: { prompt?: string[] };
}

export async function generateTasksFromPromptAction(
  prevState: GenerateTasksAIFormState,
  formData: FormData
): Promise<GenerateTasksAIFormState> {
  const session = await auth();
  if (!session?.user?.uuid) {
    return { error: "Authentication required." };
  }
  ...
  if (!projectUuid || !prompt) {
    return { error: 'Project and prompt are required.' };
  }

  const userRole = await dbGetProjectMemberRole(projectUuid, session.user.uuid);
  if (!userRole || !['owner', 'co-owner', 'editor'].includes(userRole)) {
    return { error: "You do not have permission to generate tasks in this project." };
  }
  ...
  try {
    ...
    return { message: `${createdCount} tasks were successfully generated by AI.` };
  } catch (error: any) {
    console.error('Error generating tasks with AI:', error);
    return { error: error.message || 'An unexpected error occurred during AI task generation.' };
  }
}
...
export async function getRepoContentsAction(projectUuid: string, path: string = ''): Promise<GithubRepoContentItem[] | { error: string }> {
  ...
  try {
    ...
    // Check if the repo is empty by looking for the specific message from GitHub's API
    if ('message' in data && (data as any).message === "This repository is empty.") {
        return [];
    }

    // If it's not an array and not the "empty repo" message, treat it as a single file response
    return [data as GithubRepoContentItem];
  } catch (error: any) {
    ...
    if (error.status === 404) {
      // Differentiate between an empty repo and a truly not found path.
      if (error.response?.data?.message?.includes("This repository is empty")) {
          return []; // This is a valid state, not an application error.
      }
